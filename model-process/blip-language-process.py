...
def get_attention_graph(model_name, save_to_file=False, file_path=None, device=None, cache_dir=None, max_position=50):
    ...
    # In BLIP the embedding layer is inside the BERT sub-module
    pos_embed_text = text_decoder.bert.embeddings.position_embeddings.weight[:max_position].to(device)  # [pos, dim]
    hidden_size_text = text_decoder.config.hidden_size
    num_heads_text = text_decoder.config.num_attention_heads
    head_dim_text = hidden_size_text // num_heads_text
    ...
    with torch.no_grad():
        ...
        for i, layer in enumerate(text_decoder.bert.encoder.layer):
            print(f' Text Layer {i}')
            query_weight = layer.attention.self.query.weight.to(device)
            key_weight = layer.attention.self.key.weight.to(device)

            layer_heads = []
            for h in range(num_heads_text):
                W_q = query_weight[h * head_dim_text:(h + 1) * head_dim_text, :]
                W_k = key_weight[h * head_dim_text:(h + 1) * head_dim_text, :]

                agg1 = pos_embed_text @ W_q.T
                agg2 = pos_embed_text @ W_k.T

                agg = agg1 @ agg2.T
                agg = Norm(agg, head_dim_text)
                agg = torch.nn.functional.softmax(agg, dim=-1)
                agg = safe_minmax_scale(agg)

                layer_heads.append(agg.cpu().tolist())
            text_results[i] = layer_heads  
