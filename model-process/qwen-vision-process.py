...
import json
from utils import Norm,safe_minmax_scale,process_single_model_plus,get_vit_base_pos_embedding
...
def get_attention_graph(model_name, save_to_file=False, file_path=None,cache_dir=None, device_map="auto"):  
    ...
    base_pos_embedding = get_vit_base_pos_embedding(
        seq_len=seq_len,
        target_dim=hidden_size,  # 与模型隐藏层维度匹配
        device=device
    )
    print(f"Loaded ViT base position embedding: {base_pos_embedding.shape}")
    ...
    with torch.no_grad():
        for block_idx, block in enumerate(model.visual.blocks):
            ...
            attn = block.attn
            # Get merged QKV weights
            W_qkv = attn.qkv.weight.to(device).float()  # [3*hidden_size, hidden_size]
            # Split QKV weights into Q, K, V three parts
            W_q, W_k, W_v = torch.split(W_qkv, hidden_size, dim=0)
            
            # --------------------------
            # Statistical distribution characteristics of current layer's W_Q and W_K
            # --------------------------
            # Calculate weight standard deviation
            w_q_std = torch.std(W_q)
            w_k_std = torch.std(W_k)
            # Take the average of both as reference baseline
            weight_std = (w_q_std + w_k_std) / 2
            
            # Calculate standard deviation of current base position embedding
            ...
            # Scale position encoding (separate scaling for each layer)
            pos_embedding = base_pos_embedding * scale_factor
            #print(f"Layer {layer_idx}: W_Q std={w_q_std:.4f}, W_K std={w_k_std:.4f}, "
            #      f"Position encoding scaling factor={scale_factor:.4f}")
            
            layer_results = []
            
            for h in range(num_heads):
                try:
                    # Calculate current head's index range
                    start_idx = h * head_dim
                    end_idx = (h + 1) * head_dim
                    
                    # Extract Q/K weights for single attention head
                    W_q_h = W_q[start_idx:end_idx, :]  # [head_dim, hidden_size]
                    W_k_h = W_k[start_idx:end_idx, :]  # [head_dim, hidden_size]
                    
                    # Calculate Q and K using position encoding and weights
                    ...
                    print(f"Error processing layer {block_idx}, head {h}: {e}")
                    layer_results.append(torch.zeros(seq_len, seq_len).tolist())
            
            Aggregation_result[str(block_idx)] = layer_results
            print(f"Layer {block_idx} processing completed, time taken: {time.time() - start_time:.2f} seconds")
...
if __name__ == '__main__':
    ...
    models = [
        ("Qwen/Qwen2.5-VL-3B-Instruct", [1,2]), 
        # ("model_name", [x,x,x]),  
    ]
